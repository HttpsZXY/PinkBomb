...
WindUI = tryWind("https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua")
	or tryWind("https://cdn.jsdelivr.net/gh/Footagesus/WindUI@main/dist/main.lua")
	or tryWind("https://github.com/Footagesus/WindUI/releases/download/1.6.66/main.lua")
...
pcall(function()
	-- Comic Neue (fallback to previous hub font)
...
local function Rem(name)
	...
	for _, d in ipairs(RS:GetChildren()) do
		...
		local nested = d:FindFirstChild(name, true)
		if nested and (nested:IsA("RemoteEvent") or nested:IsA("RemoteFunction")) then
			remCache[name] = nested
			return nested
		end
	end
	return nil
...
local function refreshCryptids(force)
	...
	local maxDepth = OnPhone and 2 or 3
...
local function GetMachines()
	...
	local hasCorey = false
	for _, m in ipairs(out) do
		if m.Type == "Corey" then hasCorey = true break end
	end
	if not hasCorey then
		local function walk(node, depth)
			if depth > 5 or not node then return end
			for _, c in ipairs(node:GetChildren()) do
				if c:IsA("Model") then
					local low = string.lower(c.Name)
					if low:find("coreymachine", 1, true)
...
local function CompleteRemoteOnly(tag)
	...
	local function fireTarget(target)
		if not target or not target.Parent then return end
		if IsDone(target) then return end
		pcall(function()
			rem:FireServer(target, "complete")
		end)
		local root = machineRoot(target)
		if root and root ~= target and not IsDone(root) then
			pcall(function()
				rem:FireServer(root, "complete")
			end)
		end
	end
	local any = false
	for _, m in ipairs(CS:GetTagged(tag)) do
		if m and m.Parent and not IsDone(m) then
			fireTarget(m)
			any = true
		end
	end
	for _, m in ipairs(GetMachines()) do
		if m.Tag == tag and m.Instance and m.Instance.Parent and not m.Done then
			fireTarget(m.Tagged or m.Instance)
			fireTarget(m.Instance)
			any = true
		end
	end
	if not any and tag == "CoreyMachine" then
		for _, m in ipairs(GetMachines()) do
			if m.Type == "Corey" and m.Instance and m.Instance.Parent and not m.Done then
				fireTarget(m.Tagged or m.Instance)
				fireTarget(m.Instance)
			end
		end
...
local function FireCompleteOne(obj, tag)
	...
	local function fire(t)
		if t and t.Parent and not IsDone(t) then
			pcall(function()
				rem:FireServer(t, "complete")
			end)
		end
	end
	fire(obj)
	...
	if tag then
		for _, m in ipairs(CS:GetTagged(tag)) do
			if m and m.Parent and (m == obj or m == root or m:IsDescendantOf(obj) or obj:IsDescendantOf(m)) then
				fire(m)
			end
		end
	end
...
local function farmLogRefreshUI()
	pcall(function()
		local keys = FarmLogs.keys
		if #keys == 0 then
			keys = { "None" }
		end
		if FarmLogs.uiDrop then
			pcall(function()
				if type(FarmLogs.uiDrop.Refresh) == "function" then
					FarmLogs.uiDrop:Refresh(keys)
				end
			end)
			pcall(function()
				if type(FarmLogs.uiDrop.SetValues) == "function" then
					FarmLogs.uiDrop:SetValues(keys)
				end
			end)
		end
		if FarmLogs.uiList and FarmLogs.uiList.SetDesc then
			if #FarmLogs.keys == 0 then
				FarmLogs.uiList:SetDesc("No floors logged yet.")
			else
				FarmLogs.uiList:SetDesc(table.concat(FarmLogs.keys, "\n"))
			end
		end
		if FarmLogs.uiPara and FarmLogs.uiPara.SetDesc and FarmLogs.selected then
			local e = FarmLogs.byKey[FarmLogs.selected]
			if e then
				local c = #e.cryptids > 0 and table.concat(e.cryptids, ", ") or "None"
				local i = #e.items > 0 and table.concat(e.items, ", ") or "None"
				FarmLogs.uiPara:SetDesc(string.format(
					"Mode: %s\nMap: %s\nCryptid's: %s\nItem's: %s",
					e.mode, e.map ~= "" and e.map or "?", c, i
				))
			end
		end
	end)
...
local function commitFloorLog(mode)
	mode = mode or FloorSession.mode or "Auto-Farm"
	sessionNoteCryptids()
	sessionNoteItems()
	FarmLogs.seq = (FarmLogs.seq or 0) + 1
	local floorNum = FarmLogs.seq
	if typeof(FloorTrack.current) == "number" then
		floorNum = FloorTrack.current
	elseif typeof(FloorTrack.passed) == "number" and FloorTrack.passed > 0 then
		floorNum = FloorTrack.passed
	end
	local key = string.format("Floor %s (%s)", tostring(floorNum), mode)
	local n = 2
	while FarmLogs.byKey[key] do
		key = string.format("Floor %s (%s) #%d", tostring(floorNum), mode, n)
		n = n + 1
	end
	local mapSv = RS:FindFirstChild("CurrentMap")
	local mapName = mapSv and tostring(mapSv.Value or "") or ""
	local area = RS:FindFirstChild("Area")
	if (not mapName or mapName == "") and area then
		mapName = tostring(area.Value or "")
	end
	local entry = {
		key = key,
		floor = floorNum,
		mode = mode,
		map = mapName,
		cryptids = sessionKeysSorted(FloorSession.cryptids),
		items = sessionKeysSorted(FloorSession.items),
	}
	table.insert(FarmLogs.keys, key)
	FarmLogs.byKey[key] = entry
	FarmLogs.selected = key
	farmLogRefreshUI()
	sessionReset(mode)
end
	...
	pcall(function()
		...
		for _, fx in ipairs(Lighting:GetChildren()) do
			...
			elseif fx:IsA("ColorCorrectionEffect") then
				fx.Brightness = math.max(fx.Brightness, 0.05)
				fx.Contrast = 0
				fx.Saturation = fx.Saturation
...
local LANG = {
	code = "en",
	strings = {
		en = { ready = "ready", invFull = "Inventory is full.", leave = "Leaving !!" },
		es = { ready = "listo", invFull = "Inventario lleno.", leave = "Saliendo !!" },
		pt = { ready = "pronto", invFull = "Inventario cheio.", leave = "Saindo !!" },
		tr = { ready = "hazir", invFull = "Envanter dolu.", leave = "Cikiliyor !!" },
		ar = { ready = "جاهز", invFull = "الحقيبة ممتلئة.", leave = "مغادرة !!" },
		fr = { ready = "pret", invFull = "Inventaire plein.", leave = "Depart !!" },
		de = { ready = "bereit", invFull = "Inventar voll.", leave = "Verlassen !!" },
		it = { ready = "pronto", invFull = "Inventario pieno.", leave = "Uscita !!" },
		ru = { ready = "готово", invFull = "Инвентарь полон.", leave = "Выход !!" },
		ko = { ready = "준비됨", invFull = "인벤토리 가득 참.", leave = "퇴장 !!" },
		ja = { ready = "準備完了", invFull = "インベントリがいっぱいです。", leave = "退出 !!" },
		zh = { ready = "就绪", invFull = "背包已满。", leave = "离开 !!" },
		id = { ready = "siap", invFull = "Inventaris penuh.", leave = "Keluar !!" },
		pl = { ready = "gotowe", invFull = "Ekwipunek pelny.", leave = "Wyjscie !!" },
	},
}

local function L(key)
	local pack = LANG.strings[LANG.code] or LANG.strings.en
	return (pack and pack[key]) or (LANG.strings.en[key]) or key
end

local function applyLanguage(code)
	if not code or not LANG.strings[code] then return end
	LANG.code = code
	pcall(function()
		if WindUI.SetLanguage then WindUI:SetLanguage(code) end
	end)
	pcall(function()
		if writefile then
			...
			writefile("PinkBombs_Funhouse_CFGS/_lang.ec", code)
		end
	end)
end

pcall(function()
	if isfile and isfile("PinkBombs_Funhouse_CFGS/_lang.ec") then
		local c = readfile("PinkBombs_Funhouse_CFGS/_lang.ec")
		if c and LANG.strings[c] then LANG.code = c end
	end
end)
...
local function boostPrompts(root)
	if not root then return end
	for _, d in ipairs(root:GetDescendants()) do
		if d:IsA("ProximityPrompt") then
			pcall(function()
				if not d:GetAttribute("EC_AuraSaved") then
					d:SetAttribute("EC_AuraSaved", true)
					d:SetAttribute("EC_AuraMax", d.MaxActivationDistance)
					d:SetAttribute("EC_AuraLOS", d.RequiresLineOfSight)
				end
				d.MaxActivationDistance = math.max(d.MaxActivationDistance, Features.auraRange)
...
local function restorePrompts(root)
	if not root then return end
	for _, d in ipairs(root:GetDescendants()) do
		if d:IsA("ProximityPrompt") and d:GetAttribute("EC_AuraSaved") then
			pcall(function()
				local m = d:GetAttribute("EC_AuraMax")
				if typeof(m) == "number" then d.MaxActivationDistance = m end
				local los = d:GetAttribute("EC_AuraLOS")
				if typeof(los) == "boolean" then d.RequiresLineOfSight = los end
			end)
		end
	end
end
...
local function autoPlayInputs()
	...
	pressKey(Enum.KeyCode.ButtonA)
	for _, gui in ipairs(pg:GetDescendants()) do
		if gui:IsA("GuiObject") and gui.Visible then
			local n = string.lower(gui.Name)
			if n:find("hit", 1, true) or n:find("answer", 1, true) or n:find("target", 1, true)
				or n:find("button", 1, true) or n:find("zone", 1, true) or n:find("red", 1, true) then
				local abs = gui.AbsolutePosition
				local size = gui.AbsoluteSize
				if size.X > 2 and size.Y > 2 then
					clickAt(abs.X + size.X * 0.5, abs.Y + size.Y * 0.5)
				end
			end
		end
	end
	...
	for _, n in ipairs({ "Storage", "Arcade", "Corey", "InGame" }) do
		...
		if f then
			for _, d in ipairs(f:GetDescendants()) do
				if (d:IsA("Frame") or d:IsA("ScreenGui")) and d.Visible then
					local nm = string.lower(d.Name)
					if nm:find("minigame", 1, true) or nm:find("mini", 1, true) then
						return true
					end
				end
			end
...
local function minigameTick()
	...
	if Features.minigameMode == "Force Win" then
		forceWinMinigames()
	else
		autoPlayInputs()
	end
...
local function autoCardTick()
	...
	local hearts = Hearts()
	local card = "TaskLess"
	if Features.autoBestCard or Features.autoCard then
...
local function autoUseItemTick()
	...
	local c = Char()
	if not c then return end
	local hearts = tonumber(c:GetAttribute("hearts")) or Hearts()
	local poison = tonumber(c:GetAttribute("Poison")) or 0
	local blocked = c:GetAttribute("HealingBlocked") == true
	local function useSlot(slot)
		pcall(function()
			rem:FireServer(slot, Vector3.new(0, -1, 0))
		end)
	end
	if Features.autoUseMode == "Instant" then
		for i = 1, 3 do useSlot(i) end
		return
	end
	if not blocked then
		if hearts <= 1 then useSlot(1)
		elseif hearts == 2 then useSlot(2)
		end
	end
	if poison > 0 then useSlot(1) end
end

local function itemAuraTick()
	if not Features.itemAura then return end
	local r = Root()
	if not r then return end
	local range = Features.itemAuraRange or 18
	local function tryPick(o)
		if not o or not o.Parent then return end
		if o:GetAttribute("FH_Hidden") then return end
		local low = string.lower(tostring(o.Name or ""))
		if low:find("fleshtrap", 1, true) or low:find("rottenbanana", 1, true) then return end
		local p = Part(o)
		if not p or (p.Position - r.Position).Magnitude > range then return end
		local prompt = o:FindFirstChildWhichIsA("ProximityPrompt", true)
		if not prompt then return end
		pcall(function()
			prompt.Enabled = true
			prompt.MaxActivationDistance = math.max(prompt.MaxActivationDistance, range + 8)
	...
	pcall(function()
		for _, tag in ipairs({ "Spawneditem", "SpawnedItem", "Item" }) do
			for _, o in ipairs(CS:GetTagged(tag)) do
				tryPick(o)
			end
		end
	...
	pcall(function()
		for _, o in ipairs(Workspace:GetChildren()) do
			if isBringItem and isBringItem(o) then tryPick(o) end
		end
	end)
end

local function machineAuraTick()
...
local function autoMachinesTick()
	...
	if not SafeTP(best.Front, best.Position, Settings.safeDist or 42) then return end
	task.wait(0.12)
...
local function escapeCryptidsTick()
	if not Features.escapeCryptids then return end
	if isIntermission() or not Alive() then return end
	local r = Root()
	if not r then return end
	local chased = Char() and Char():GetAttribute("Chased") == true
	local near = CryptidNear(r.Position, 28, false)
	if chased or near then
		local origin = r.Position
		goSafe()
		task.wait(1.2)
		if Features.escapeCryptids and Alive() and not isIntermission() then
			SafeTP(origin, nil, 1)
		end
	end
end
...
local function auraTick()
	if not Features.aura then return end
	boostPrompts(Workspace)
	local c = Char()
	if c then boostPrompts(c) end
end
	...
	if kind == "Machines" then
		for _, m in ipairs(GetMachines()) do
			if m.Instance and m.Position then
				local label = prettyMachineName(m.Instance) .. (m.Done and " (Complete)" or " (Incomplete)")
				table.insert(out, { label = label, pos = m.Front or m.Position, look = m.Position })
			end
		end
	else
		local seen = {}
		local function add(o)
			...
			local p = Part(o)
			if p then
				table.insert(out, { label = tostring(o.Name):gsub("%d+$", ""), pos = p.Position + Vector3.new(0, 3, 0), look = p.Position })
			end
		end
		pcall(function()
			for _, tag in ipairs({ "Spawneditem", "SpawnedItem" }) do
...
local function doTeleportTo(entry)
	...
	if Features.tpMode == "Cryptid Check" and CryptidNear(entry.pos, rad, true) then
		Ping("Teleport", "Blocked by cryptid", 2)
		return
	end
	SafeTP(entry.pos, entry.look, rad)
end
...
local ESP = {
	...
	itemHooked = false,
	blItem = {},
	maxObjects = OnPhone and 36 or 72,
	maxDistance = OnPhone and 220 or 300,
	scanInterval = OnPhone and 3.2 or 2.4,
}
			...
			if on and not isEspHiddenMachine(m.Type) then
				...
				if ESP.tags.Progress or Features.progressESP then
					...
					if prog and need and need > 0 then
						...
						table.insert(parts, m.Done and "Complete" or "Incomplete")
					end
				end
				if (ESP.tags.Minigame or Features.minigameESP) then
					...
					if c then
						...
						if inMini == true or inMini == 1 then
							table.insert(parts, "In minigame")
						end
					end
				end
...
local okWin, Hub = pcall(function()
	return WindUI:CreateWindow({
		...
		Size = OnPhone and UDim2.fromOffset(430, 600) or UDim2.fromOffset(580, 460),
		MinSize = OnPhone and Vector2.new(400, 540) or Vector2.new(300, 300),
		MaxSize = Vector2.new(1200, 900),
		Transparent = true,
		Theme = "Dark",
		Resizable = true,
		SideBarWidth = OnPhone and 175 or 200,
		HideSearchBar = true,
...
local function Add(sec, method, data)
	...
	if not ok then warn("[Pink bomb's][UI]", method, r) end
	return r
end

pcall(function()
	if isfile and isfile("PinkBombs_Funhouse_CFGS/_lang.ec") then return end
	if WindUI and WindUI.Popup then
		WindUI:Popup({
			Title = "Select language",
			Content = "Choose the language for this hub.",
			Buttons = {
				{ Title = "English", Callback = function() applyLanguage("en") end },
				{ Title = "Espanol", Callback = function() applyLanguage("es") end },
				{ Title = "More in Config", Callback = function() end },
			},
		})
	end
end)
...
safeCall("UI:Automatic", function()
	...
	local function runInstant(tag, typeName)
		MachineManager.Clear()
		CompleteRemoteOnly(tag)
		for _, m in ipairs(GetMachines()) do
			if m.Type == typeName and m.Instance and m.Instance.Parent and not m.Done then
	...
	Add(s, "Toggle", {
		...
		Callback = function(v)
			...
			Loop("AC_A", function() return acA end, function()
				runInstant("ArcadeMachine", "Arcade")
			end, 0.45)
		end,
	})
	Add(s, "Toggle", {
		...
		Callback = function(v)
			...
			Loop("AC_S", function() return acS end, function()
				runInstant("StorageMachine", "Storage")
			end, 0.4)
		end,
	})
	Add(s, "Toggle", {
		...
		Callback = function(v)
			...
			Loop("AC_C", function() return acC end, function()
				runInstant("CoreyMachine", "Corey")
			end, OnPhone and 0.55 or 0.4)
		end,
	})

	local mc = Sec(t, "Machine Complete Modes", nil, "solar:gamepad-bold")
	Add(mc, "Dropdown", {
		Title = "Minigame Method",
		Values = { "Force Win", "Auto-Play Inputs" },
		Value = Features.minigameMode,
		Callback = function(v)
			if type(v) == "string" and v ~= "" then Features.minigameMode = v end
		end,
	})
	Add(mc, "Toggle", {
		Title = "Auto Play Mini-Games",
		Desc = "Only while inside a machine minigame (PC/Mobile).",
		Value = false,
		Callback = function(v)
			Features.minigameOn = v == true
			if not v then Stop("MiniA") return end
			Loop("MiniA", function() return Features.minigameOn end, minigameTick, 0.35)
		end,
	})

	local auto = Sec(t, "Auto Systems", nil, "solar:bolt-circle-bold")
	Add(auto, "Toggle", {
		Title = "Auto Machines",
		Value = false,
		Callback = function(v)
			...
			Loop("AutoMach", function() return Features.autoMachines end, autoMachinesTick, 0.7)
		end,
	})
	Add(auto, "Toggle", {
		Title = "Auto Exit",
		...
		Callback = function(v)
			...
			Loop("AutoEx", function() return Features.autoExit end, autoExitTick, 1.0)
		end,
	})
	Add(auto, "Toggle", {
		Title = "Escape Cryptids",
		Value = false,
		Callback = function(v)
			Features.escapeCryptids = v == true
			if not v then Stop("EscCry") return end
			Loop("EscCry", function() return Features.escapeCryptids end, escapeCryptidsTick, 0.45)
		end,
	})
	Add(auto, "Toggle", {
		Title = "Auto Card",
		Desc = "Votes in intermission.",
		Value = false,
		Callback = function(v)
			Features.autoCard = v == true
			if not v then Stop("AutoCard") return end
			Loop("AutoCard", function() return Features.autoCard end, autoCardTick, 1.2)
		end,
	})
	Add(auto, "Toggle", {
		...
		Callback = function(v)
			...
			if not v then Stop("AutoBestCard") return end
			Loop("AutoBestCard", function() return Features.autoBestCard end, autoCardTick, 1.2)
		end,
	})
	Add(auto, "Toggle", {
		Title = "Auto Use Item",
		...
		Callback = function(v)
			...
			Loop("AutoUse", function() return Features.autoUseItem end, autoUseItemTick, 0.8)
		end,
	})
	Add(auto, "Dropdown", {
		Title = "Auto Use Behavior",
		Values = { "Advanced", "Instant" },
		Value = Features.autoUseMode,
		Callback = function(v)
			if type(v) == "string" then Features.autoUseMode = v end
		end,
	})
	Add(auto, "Toggle", {
		Title = "Item Aura",
		Desc = "Auto-pick nearby items (prompt). No TP.",
		Value = false,
		Callback = function(v)
			Features.itemAura = v == true
			if not v then Stop("ItAura") return end
			Loop("ItAura", function() return Features.itemAura end, itemAuraTick, 0.45)
		end,
	})
	Add(auto, "Toggle", {
		Title = "Machine Aura",
		Desc = "Extends machine prompt range only (no auto-complete).",
		Value = false,
		Callback = function(v)
			Features.machineAura = v == true
			if not v then Stop("McAura") return end
			Loop("McAura", function() return Features.machineAura end, machineAuraTick, 0.55)
		end,
	})
	Add(auto, "Slider", {
	...
	local auraSec = Sec(t, "Aura", nil, "solar:radar-2-bold")
	Add(auraSec, "Toggle", {
		Title = "Prompt Aura",
		Value = false,
		Callback = function(v)
			Features.aura = v == true
			if not v then
				Stop("Aura")
				restorePrompts(Workspace)
				return
			end
			Loop("Aura", function() return Features.aura end, auraTick, 1.2)
		end,
	})
	Add(auraSec, "Slider", {
		Title = "Aura Range",
		Value = Features.auraRange,
		Min = 8,
		Max = 40,
		Step = 1,
		Callback = function(v)
			Features.auraRange = tonumber(v) or 18
		end,
	})
...
safeCall("UI:Farms", function()
	...
	local logs = Sec(t, "Farm Logs", nil, "solar:document-text-bold")
	local logList = Add(logs, "Paragraph", {
		Title = "Logged Floors",
		Desc = "No floors logged yet.",
	})
	local logPara = Add(logs, "Paragraph", {
		Title = "Log Details",
		Desc = "Pick a floor in Log to see Cryptid's and Item's.",
	})
	local logDrop = Add(logs, "Dropdown", {
		Title = "Log",
		Values = { "None" },
		Value = "None",
		Callback = function(v)
			if type(v) ~= "string" or v == "None" or v == "" then
				FarmLogs.selected = nil
				if logPara and logPara.SetDesc then
					logPara:SetDesc("Pick a floor in Log to see Cryptid's and Item's.")
				end
				return
			end
			FarmLogs.selected = v
			local e = FarmLogs.byKey[v]
			if e and logPara and logPara.SetDesc then
				local c = #e.cryptids > 0 and table.concat(e.cryptids, ", ") or "None"
				local i = #e.items > 0 and table.concat(e.items, ", ") or "None"
				logPara:SetDesc(
					"Mode: " .. tostring(e.mode)
					.. "\nMap: " .. tostring((e.map ~= "" and e.map) or "?")
					.. "\nCryptid's: " .. c
					.. "\nItem's: " .. i
				)
			end
	...
	FarmLogs.uiDrop = logDrop
	FarmLogs.uiPara = logPara
	FarmLogs.uiList = logList
	...
	Add(bring, "Toggle", {
		...
		Value = true,
		Callback = function(v) Bring.coreyRoutine = v ~= false end,
	})
	Add(bring, "Toggle", {
		Title = "Prompt Before Remote",
		Value = true,
		Callback = function(v) Bring.usePrompt = v ~= false end,
	})
	Add(bring, "Toggle", {
...
safeCall("UI:Utility", function()
	...
	Add(help, "Button", {
		...
		Desc = "GameTP to in-dev place. If blocked, use IY GameTP ID: 127752496426569. Delta: disable Verify Teleport.",
		Callback = function()
			pcall(function()
				game:GetService("TeleportService"):Teleport(127752496426569, Me)
			end)
		end,
	})
...
safeCall("UI:Config", function()
	...
	Add(perf, "Dropdown", {
		...
		Callback = function(v)
			if v == "Low-end" then
				ESP.maxObjects = 14
				ESP.scanInterval = 3.2
			elseif v == "High-end" then
				ESP.maxObjects = 36
				ESP.scanInterval = 1.2
			else
				ESP.maxObjects = 22
				ESP.scanInterval = 2.0
			end
		end,
	})
	local lang = Sec(t, "Language", nil, "solar:global-bold")
	local langCodes = { "en", "es", "pt", "tr", "ar", "fr", "de", "it", "ru", "ko", "ja", "zh", "id", "pl" }
	Add(lang, "Dropdown", {
		Title = "Language",
		Values = langCodes,
		Value = LANG.code,
		Callback = function(v)
			if type(v) == "string" then applyLanguage(v) end
		end,
	})
