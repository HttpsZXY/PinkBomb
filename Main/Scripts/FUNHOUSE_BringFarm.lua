...
local function w(...) pcall(warn, "[Pink Bomb]", ...) end
...
local function FireAny(inst, ...)
	...
	pcall(function()
		if inst:IsA("RemoteEvent") then inst:FireServer(...)
		elseif inst:IsA("BindableEvent") then inst:Fire(...)
		elseif inst:IsA("RemoteFunction") then inst:InvokeServer(...)
		elseif inst:IsA("BindableFunction") then inst:Invoke(...) end
	end)
end
