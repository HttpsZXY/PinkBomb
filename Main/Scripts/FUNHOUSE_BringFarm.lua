...
local S = {
	...
	farmDelay = 0.35, taskWait = 0.08, godOn = false, godArmed = true, chased = false,
...
local function sendWebhook(content)
	...
	if fluxus and fluxus.request then
		if tryPost(function()
			fluxus.request({ Url = S.webhookUrl, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = body })
		end) then return true end
	end
	local ok = pcall(function()
...
local function sendMapStartWebhook(force)
	...
	task.spawn(function()
		...
		if not ok then
			...
			task.wait(1)
			sendWebhook(formatFloorEmbed(entry))
		end
	end)
	return true
end
...
local function completeMachine(m)
	...
	local tries = isCorey and 8 or 6
	local allowPlayerTP = not (S.mode == "Bring" and not InCoreyZone())
	for i = 1, tries do
		local pos = m.Front or m.Position
		if pos and isCorey and allowPlayerTP then
			SoftTP(pos)
			task.wait(0.04)
			SoftTP(pos + Vector3.new(0, 1.5, 0))
		end
		if S.mode == "Bring" and not InCoreyZone() then
			local r = Root()
			if r then forceBring(obj, r.Position + Vector3.new(0, 1, 0)) end
		end
...
local function forceHearts(n)
	...
	if c then
		for _, k in ipairs({ "hearts", "Hearts", "Heart", "heart", "Lives", "lives" }) do
			pcall(function() c:SetAttribute(k, n) end)
		end
		for _, child in ipairs(c:GetChildren()) do
			if child:IsA("ValueBase") then
				local ln = string.lower(child.Name)
				if ln:find("heart", 1, true) or ln == "lives" then
					pcall(function() child.Value = n end)
				end
			end
		end
	end
	for _, k in ipairs({ "hearts", "Hearts", "Heart" }) do
		pcall(function() Me:SetAttribute(k, n) end)
	end
end
...
local function protectHumanoid(h)
	...
	pcall(function()
		table.insert(_godHumanoidConn, h.HealthChanged:Connect(function(hp)
			if not S.godOn and not (S.godArmed and Hearts() <= 1) then return end
	...
	pcall(function()
		table.insert(_godHumanoidConn, h.Died:Connect(function()
	...
	pcall(function()
		table.insert(_godHumanoidConn, h:GetPropertyChangedSignal("Health"):Connect(function()
			if not S.godOn and not (S.godArmed and Hearts() <= 1) then return end
	...
	pcall(function()
		...
		if newcclosure then
			mt.__namecall = newcclosure(function(self, ...)
				...
				if S.godOn or (S.godArmed and Hearts() <= 1) then
					local m = string.lower(tostring(method))
					if m == "takedamage" or m == "breakjoints" or m == "destroy" then
						if typeof(self) == "Instance" and (self:IsA("Humanoid") or self == Char() or (Char() and self:IsDescendantOf(Char()))) then
							return
						end
					end
				end
...
local function godProtectTick()
	if not S.godArmed and not S.godOn then return end
	local hp = Hearts()
	if S.godOn or hp <= 1 then
		forceHearts(math.max(hp, 1))
		local h = Hum()
		if h then
			pcall(function()
		...
		if c then
			pcall(function()
				for _, d in ipairs(c:GetDescendants()) do
					if d:IsA("BoolValue") and string.lower(d.Name):find("dead", 1, true) then
						d.Value = false
					end
				end
			end)
...
local function tryGodMode()
	...
	if hp <= 1 and not S.godOn then
		...
		pcall(function()
			...
			if h then
				h.Health = 0
				pcall(function() h:TakeDamage(99999) end)
			end
		end)
		task.wait(0.2)
		pcall(function() Me:LoadCharacter() end)
		...
		pcall(function()
			...
			for _, d in ipairs(c:GetDescendants()) do
				if d:IsA("Accessory") or d:IsA("Shirt") or d:IsA("Pants") or d:IsA("ShirtGraphic") or d:IsA("BodyColors") then
...
local function forceBring(obj, dest)
	...
	local target = Vector3.new(dest.X, dest.Y + 2, dest.Z + 3)
	...
	local ok = pcall(function()
		...
		if root:IsA("Model") then
			...
			if pp then
				local base = pp.Position
				pp.CFrame = cf
				for _, d in ipairs(root:GetDescendants()) do
					if d:IsA("BasePart") and d ~= pp then
						local off = d.Position - base
						d.CFrame = cf * CFrame.new(off)
					end
				end
			end
		else
