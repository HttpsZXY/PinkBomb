...
local function loadAsset(url, name)
	...
	task.spawn(function()
		...
		if not ok or type(data) ~= "string" or #data < 64 then return end
		local fname = "PinkBomb_" .. tostring(name or "img") .. ".png"
		pcall(function()
...
local function w(...)
	local parts = { "[Pink Bomb]" }
	for i = 1, select("#", ...) do
		parts[#parts + 1] = tostring(select(i, ...))
	end
	warn(table.unpack(parts))
end
...
-- drag + minimize wired after pages
...
local fullSize = UDim2.new(0, 320, 0, 440)
MinBtn.ZIndex = 5
Close.ZIndex = 5
...
do
	...
	dragBtn.Size = UDim2.new(1, -100, 0, 34)
...
local function Alive()
	...
	local hp = Hearts()
	if h.Health <= 0 and (hp == nil or hp <= 0) then return false end
	if Me:GetAttribute("IsDead") == true and (hp == nil or hp <= 0) then return false end
	return true
end

local remCache = {}
local function isRemote(r)
	return r and (r:IsA("RemoteEvent") or r:IsA("RemoteFunction") or r:IsA("BindableEvent") or r:IsA("BindableFunction"))
end
...
local S = {
	...
	farmDelay = 0.32, taskWait = 0.05, godOn = false, godArmed = true, chased = false,
...
local function GetMachines()
	...
	if not (hasA and hasS and hasC) then
		local function walk(node, depth)
			if depth > 7 or not node then return end
...
local function FirePrompt(obj)
	...
	if root and root ~= obj then collect(root) end
...
local function FireComplete(tag, machine)
	...
	local root = machineRoot(machine) or machine
	local targets = { machine }
	if root ~= machine then targets[2] = root end
	local function fire(name, ...)
		local rem = Remote(name)
	...
	if tag == "ArcadeMachine" or tag == "Arcade" then
		fire("ArcadeMachineUse", "complete")
		fire("ArcadeMachineUse")
		fire("ArcadeForceComplete")
	elseif tag == "CoreyMachine" or tag == "Corey" then
		fire("CoreyTreeComplete")
		fire("CoreyTreeComplete", true)
		fire("StorageMachineUse", "complete")
		fire("ArcadeForceComplete")
	else
		fire("StorageMachineUse", "complete")
		fire("StorageMachineUse")
		fire("ArcadeForceComplete")
	end
end
...
local function InIntermission()
	...
	if gs and gs:IsA("ValueBase") and tostring(gs.Value) == "Intermission" then return true end
	return false
end
...
local function sendWebhookRaw(content)
	...
	local function okStatus(res)
		...
		local code = res.StatusCode or res.Status or res.status_code or res.statusCode
		if code == nil then return true end
		code = tonumber(code) or 0
		return code == 200 or code == 204 or code == 201
	end
	local tried = false
	if request then
		tried = true
		local ok, res = pcall(function()
			return request({ Url = S.webhookUrl, Method = "POST", Headers = headers, Body = body })
		end)
	...
	if http_request then
		tried = true
		local ok, res = pcall(function()
			return http_request({ Url = S.webhookUrl, Method = "POST", Headers = headers, Body = body })
		end)
		if ok and okStatus(res) then return true end
	end
	if syn and syn.request then
		tried = true
		local ok, res = pcall(function()
			return syn.request({ Url = S.webhookUrl, Method = "POST", Headers = headers, Body = body })
		end)
		if ok and okStatus(res) then return true end
	end
	local ok = pcall(function()
		HttpService:PostAsync(S.webhookUrl, body, Enum.HttpContentType.ApplicationJson)
	end)
	return ok
end

local function pumpWebhookQueue()
	if _whBusy then return end
	_whBusy = true
	task.spawn(function()
		while #_whQueue > 0 do
			local item = table.remove(_whQueue, 1)
			if item and item.content then
				local dedupe = item.key
				if dedupe and _whRecent[dedupe] and (os.clock() - _whRecent[dedupe]) < 8 then
					-- skip duplicate
				else
					if dedupe then _whRecent[dedupe] = os.clock() end
					local ok = sendWebhookRaw(item.content)
					if not ok then
						w("webhook failed")
					end
					task.wait(0.85)
...
local function sendWebhook(content, key)
	if not S.webhookUrl or S.webhookUrl == "" then return false end
	if type(content) ~= "string" or content == "" then return false end
	table.insert(_whQueue, { content = content, key = key })
	pumpWebhookQueue()
	return true
end
...
local function sendMapStartWebhook(force)
	...
	if type(scanFloorSnap) == "function" then
		pcall(function() scanFloorSnap(floor) end)
	end
	local entry = {
		floor = floor,
		cryptids = (#(currentSnap.cryptids or {}) > 0) and table.concat(currentSnap.cryptids, ", ") or "",
		items = (#(currentSnap.items or {}) > 0) and table.concat(currentSnap.items, ", ") or "",
		time = currentSnap.t0 > 0 and math.floor(os.clock() - currentSnap.t0) or 0,
		god = S.godUses or 0,
		deaths = S.deaths or 0,
	}
	S.mapStartSent[key] = true
	-- never send intermission junk
	if InIntermission() and not force then return false end
	sendWebhook(formatFloorEmbed(entry), key)
	return true
end
...
local function sendMachinesRemainingWebhook(left)
	...
	local floor = ReadFloor() or S.lastFloor or 0
	local key = "left_" .. tostring(floor) .. "_" .. tostring(left)
	sendWebhook("Machines remaining: " .. tostring(left), key)
end


local function isJunkScanName(low)
	if not low then return true end
	for _, bad in ipairs({
		"intermission", "waiting", "tutorial", "lobby", "spawn", "safe",
		"table", "chair", "tv", "ground", "decal", "mapyes", "waypoint",
		"camera", "lighting", "folder", "part", "mesh", "attachment",
		"grinwelltable", "intermissiongrinwell", "dialogue", "billboard",
		"surfacegui", "screen", "ui", "button", "frame",
	}) do
		if low:find(bad, 1, true) then return true end
	end
	return false
end

local function collectNames(patterns, deep, requireHumanoid)
	local names, seen = {}, {}
	local function consider(m)
		if not m or seen[m.Name] then return end
		if not m:IsA("Model") then return end
		local low = string.lower(m.Name)
		if isJunkScanName(low) then return end
		-- skip map/lobby folders
		local parent = m.Parent
		if parent then
			local pn = string.lower(parent.Name)
			if pn:find("intermission", 1, true) or pn:find("waiting", 1, true) or pn:find("tutorial", 1, true) then
				return
			end
		end
		local match = false
		for _, p in ipairs(patterns) do
			if low:find(p, 1, true) then match = true break end
		end
		if not match then return end
		if requireHumanoid then
			if not m:FindFirstChildOfClass("Humanoid") and not m:FindFirstChild("HumanoidRootPart") then
				-- still allow tagged cryptids without humanoid
				local tagged = false
				pcall(function()
					if CS:HasTag(m, "Cryptid") or CS:HasTag(m, "Entity") then tagged = true end
				end)
	...
	-- only top-level + one level under map folders, skip intermission roots
	for _, m in ipairs(Workspace:GetChildren()) do
		local ln = string.lower(m.Name)
		if ln:find("intermission", 1, true) or ln:find("waitingfornext", 1, true) or ln == "tutorial" then
			-- skip lobby
		else
			consider(m)
...
scanFloorSnap = function(forceFloor)
	if InIntermission() or IsLoading() then
		return
	end
	local cryptids = collectNames({ "cryptid", "snuggle", "triplet", "entity", "polly", "chaser", "freddie" }, true, true)
	local items = collectNames({ "remnant", "pickup", "loot", "flashlight", "battery", "keyitem" }, true, false)
	-- filter item list more: only if looks like pickup
	local cleanItems = {}
	for _, n in ipairs(items) do
		local low = string.lower(n)
		if not isJunkScanName(low) and not low:find("machine", 1, true) then
			table.insert(cleanItems, n)
		end
	end
	if #cryptids > 0 then currentSnap.cryptids = cryptids end
	if #cleanItems > 0 then currentSnap.items = cleanItems end
	if #cryptids == 0 and #(currentSnap.cryptids or {}) == 0 then
		currentSnap.cryptids = {}
	end
	if currentSnap.t0 == 0 then currentSnap.t0 = os.clock() end
	if forceFloor then currentSnap.floor = forceFloor end
end
...
local function voteBestCard()
	...
	local floor = ReadFloor() or 0
	local key = "vote_" .. tostring(floor) .. "_" .. tostring(math.floor(os.clock() / 30))
	if S.lastVoteKey == key then return end
...
local function completeMachine(m)
	...
	local useBring = (tostring(S.mode) == "Bring") and not isCorey
	...
	local tries = isCorey and 4 or 3
	for i = 1, tries do
		if not S.running then return false end
		if useBring then
			local r = Root()
			if r then
				local spot = r.Position + Vector3.new(0, 1.5, 0)
				forceBring(obj, spot)
				if target ~= obj then forceBring(target, spot) end
			end
		elseif isCorey then
		...
		if isCorey then
			...
			FireAny(Remote("CoreyTreeComplete"), true)
			FireAny(Remote("CoreyTreeComplete"))
		end
		if i == 1 or i == tries then
			ForceRelease()
		end
		-- touch interest fallback
		pcall(function()
			local r = Root()
			if not r then return end
			for _, d in ipairs(obj:GetDescendants()) do
				if d:IsA("BasePart") and (d.Name == "Touch" or d.Name == "Hitbox" or d:FindFirstChildOfClass("TouchTransmitter")) then
					if firetouchinterest then
						firetouchinterest(r, d, 0)
						firetouchinterest(r, d, 1)
					end
				end
			end
		end)
		task.wait(0.06 + S.taskWait)
...
local _godHooks = false
local _godHumanoidConn = {}
local _godLastHum = nil
local _godCharHook = false

local function clearGodConns()
	for _, c in ipairs(_godHumanoidConn) do
		pcall(function() c:Disconnect() end)
	end
	for i = #_godHumanoidConn, 1, -1 do _godHumanoidConn[i] = nil end
	_godLastHum = nil
end

local function forceHearts(n)
	n = n or 1
	local c = Char()
	if c then
		pcall(function() c:SetAttribute("hearts", n) end)
		pcall(function() c:SetAttribute("Hearts", n) end)
	end
	pcall(function() Me:SetAttribute("hearts", n) end)
end

local function protectHumanoid(h)
	if not h or h == _godLastHum then return end
	clearGodConns()
	_godLastHum = h
	pcall(function()
	...
	local ok1, c1 = pcall(function()
		return h.HealthChanged:Connect(function(hp)
	...
	if ok1 and c1 then table.insert(_godHumanoidConn, c1) end
	local ok2, c2 = pcall(function()
		return h.Died:Connect(function()
			if not S.godOn and not S.godArmed then return end
			pcall(function()
				h.Health = h.MaxHealth
	...
	if ok2 and c2 then table.insert(_godHumanoidConn, c2) end
end

local function installGodHooks()
	if _godHooks then return end
	_godHooks = true
	-- Safe remote listeners only (NO metatable — crashes many executors)
	pcall(function()
		local rem = Remote("EntityHitLocal")
		if rem and rem:IsA("RemoteEvent") then
			connect(rem.OnClientEvent, function()
				if not S.godOn then return end
				local h = Hum()
...
local function godProtectTick()
	...
	pcall(function()
		if Me:GetAttribute("IsDead") == true then
			Me:SetAttribute("IsDead", false)
		end
	end)
end

local function tryGodMode()
	if not S.godArmed then return end
	local hp = Hearts()
	if hp == nil then return end
	if hp > 1 and not S.godOn then return end
	if hp <= 1 and not S.godOn then
		S.godOn = true
		...
		protectHumanoid(Hum())
		forceHearts(1)
		if not _godCharHook then
			_godCharHook = true
			connect(Me.CharacterAdded, function(c)
				task.wait(0.25)
				refreshChar(c)
				if S.godOn then
					protectHumanoid(Hum())
...
local function onFloorChange(newFloor)
	...
	S.floorGeneration = (S.floorGeneration or 0) + 1
	local gen = S.floorGeneration
	task.delay(0.6, function()
		if gen ~= S.floorGeneration then return end
		if type(scanFloorSnap) == "function" then scanFloorSnap(newFloor) end
	end)
...
forceBring = function(obj, dest)
	...
	local target = Vector3.new(dest.X, dest.Y + 2, dest.Z + 2.5)
	...
	local ok = pcall(function()
		...
		for _, d in ipairs(parts) do
			saved[d] = { Anchored = d.Anchored, CanCollide = d.CanCollide }
			d.Anchored = true
			d.CanCollide = false
	...
	task.delay(2.5, function()
		for d, st in pairs(saved) do
			pcall(function()
				if d and d.Parent then
					d.Anchored = st.Anchored
					d.CanCollide = st.CanCollide
				end
			end)
		end
	end)
...
local function doMachine(target)
	...
	local corey = InCoreyZone()
	-- Bring ONLY when mode is Bring and NOT in Corey
	local useBring = (tostring(S.mode) == "Bring") and (not corey)
	local ok = false
	busyTick()
	if useBring then
		S.status = "Bring " .. (target.Type or "Machine")
		local r = Root()
		if not r then return end
		-- NEVER SoftTP player to machine in Bring mode
		local bringConn
		local function pullNow()
			local rr = Root()
			if not rr or not target.Instance then return end
			local spot = rr.Position + Vector3.new(0, 1.5, 0)
			forceBring(target.Instance, spot)
			if target.Tagged then forceBring(target.Tagged, spot) end
		end
		for _ = 1, 8 do
		...
		if bringConn then pcall(function() bringConn:Disconnect() end) end
	else
		S.status = (corey and "Corey " or "Farm ") .. (target.Type or "Machine")
		local dest = target.Front or target.Position
		if dest then
			SoftTP(dest)
			task.wait(0.04)
			SoftTP(dest + Vector3.new(0, 1.5, 0))
			task.wait(0.03)
			SoftTP(dest + Vector3.new(1, 1, 0))
			SoftTP(dest)
		end
		...
		task.wait(0.04 + S.taskWait)
		ok = completeMachine(target)
		if not ok then
			if dest then
				SoftTP(dest + Vector3.new(0, 2, 0))
				task.wait(0.04)
			end
			ForceRelease()
			ok = completeMachine(target)
...
local function blockAlertsTick()
	-- non-destructive: do not Destroy RS remotes
end
...
local function farmOnce()
	...
	if not MapReady(machines) then
		S.status = "Loading Map"
		S.mapWaitStart = S.mapWaitStart or os.clock()
		if os.clock() - S.mapWaitStart > 15 then
			S.status = "Map timeout"
			S.mapWaitStart = nil
			S.loadGrace = 0
		else
...
fy = addBtn(PageFeed, fy, "Test Webhook", function()
	...
	local ok = sendWebhook("**" .. displayName() .. "** connected Pink Bomb farm.", "test_wh")
