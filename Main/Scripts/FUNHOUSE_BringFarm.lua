...
local function completeMachine(m)
	...
	local useBring = (S.mode == "Bring") and not isCorey and not InCoreyZone()
...
local function doMachine(target)
	...
	if useBring then
		...
		if not r then return end
		local spot = r.Position + Vector3.new(0, 1.5, 0)
		-- pull machine hard to player, never TP player to machine
		for _ = 1, 6 do
		...
		for _ = 1, 4 do
			...
			r = Root()
			if r then
				spot = r.Position + Vector3.new(0, 1.5, 0)
				forceBring(target.Instance, spot)
			end
