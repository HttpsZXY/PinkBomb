...
local function w(...)
	local a = { "[Pink Bomb]", ... }
	pcall(function() warn(table.unpack(a)) end)
end
...
local function InIntermission()
	...
	if inter and inter:IsA("ValueBase") and tonumber(inter.Value) and tonumber(inter.Value) > 0 then return true end
...
local function onFloorChange(newFloor)
	...
	task.delay(2.5, function() scanFloorSnap(newFloor) end)
