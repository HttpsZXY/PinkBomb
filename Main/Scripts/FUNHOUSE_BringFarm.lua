...
local function BringSpot()
	local r = Root()
	if r then
		return r.Position + Vector3.new(0, 1, 0)
	end
	return SafeSpot()
end
...
local function doMachine(target)
	...
	if useBring then
		...
		if not spot then
			local r = Root()
			spot = r and r.Position or target.Front
		end
		SoftTP(spot)
		task.wait(0.05)
		for _ = 1, 4 do
		...
		local r = Root()
		if r then SoftTP(r.Position) end
	else
...
local function farmOnce()
	...
	if InIntermission() then S.status = "Intermission" voteBestCard() bloodDonateTick() return end
