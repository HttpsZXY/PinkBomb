...
local function InCoreyZone()
	...
	if cm and cm:IsA("ValueBase") and string.lower(tostring(cm.Value)):find("corey", 1, true) then
		return true
	end
...
local function GoSafe()
	...
	if InIntermission() then return false end
	local p = SafeSpot()
	if p then return SoftTP(p) end
	return false
end
...
local function formatFloorEmbed(entry)
	...
	return table.concat({
		...
		"Cryptid's: " .. (entry.cryptids ~= "" and entry.cryptids or "-"),
		"Item's: " .. (entry.items ~= "" and entry.items or "-"),
	}, "\n")
end

local function sendMapStartWebhook()
	...
	if S.mapStartSent[key] then return end
...
local function sendMachinesRemainingWebhook(left)
	...
	task.spawn(function()
		sendWebhook("Machines remaining: " .. tostring(left))
	end)
end
...
local function tryGodMode()
	...
	if not ok then S.godOn = false return end
	task.wait(0.28)
	pcall(function() Me:LoadCharacter() end)
	task.wait(0.4)
	refreshChar(Me.Character)
	if Hearts() > 1 or not Alive() then
		S.godOn = false
	end
end
...
local function onFloorChange(newFloor)
	...
	task.delay(2.5, function() scanFloorSnap(newFloor) sendMapStartWebhook() end)
